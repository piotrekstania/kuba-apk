...
| W `dane/kopie/` zostaje **pięć ostatnich** kopii, starsze kasują się same | kopia powstaje przed **każdą** aktualizacją, a wydań bywa kilkanaście dziennie — przy ~1 MB na kopię (u brata z obrębami całej Polski ~5 MB, bo tyle waży wtedy baza) katalog rósłby bez końca. Ratunkowa jest zawsze ostatnia: gdy aktualizacja coś zepsuje, widać to od razu, a nie po dziesięciu wydaniach. Limit siedzi w `ILE_KOPII` (`aktualizacja.py`) i `ILE_KOPII_BAZY` (`db.py`) — kopie sprzed migracji schematu sprząta `db`, bo to pojedyncze pliki, a nie katalogi. Sprzątanie jest samonaprawcze: instalacja z dwudziestoma starymi kopiami schodzi do pięciu przy najbliższej aktualizacji |

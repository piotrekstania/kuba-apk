...
| **Word ma limit czasu: po 3 minutach jednego kroku strażnik zamyka naszą instancję** (`pdf._Straznik`, `LIMIT_WORDA`) | Word bez okna potrafi stanąć na pytaniu, którego nikt nie widzi (aktywacja Office, naprawa pliku). Konwersja czekała wtedy w nieskończoność pod `_BLOKADA_KONWERSJI` — stawały podglądy i każde następne składanie, aż do zamknięcia programu, a w tle zostawał `WINWORD.EXE`. Zamykamy wyłącznie proces, który po naszym `DispatchEx` pojawił się jako **jedyny nowy** (numery z `tasklist`, zdjęcie przed startem i po nim): dwa nowe naraz to znak, że brat właśnie otworzył swojego Worda, i wtedy nie zamykamy niczego — lepiej czekać niż zamknąć mu niezapisany dokument. Limit liczy się na krok (start z pierwszym dokumentem, potem każdy dokument) z ogromnym zapasem wobec zwykłych 1,5 s. Po zawieszeniu wsad nie próbuje dokumentów pojedynczo Wordem — każdy start stanąłby na tym samym oknie — tylko LibreOffice'em, jeśli jest. Kosztuje to dwa wywołania `tasklist` na jedno uruchomienie Worda. Na prawdziwym Wordzie sprawdza to `test_limit_czasu_zamyka_tylko_naszego_worda` |
| **Składanie PDF-a chodzi w puli wątków** (`run_in_threadpool` w `POST /scal/{nazwa}`) | trasa `async` wołała konwersję i sklejanie wprost w pętli zdarzeń, więc dopóki Word pracował, serwer nie odpowiadał na nic — nawet na otwarcie Pomocy w drugiej karcie — i program wyglądał na zawieszony. Pozostałe trasy z konwersją są zwykłymi `def`, które FastAPI i tak puszcza w puli. **Trasa `async` nie woła niczego, co może trwać** |
...
| `test_operaty.py`, `test_pdf.py` | nazwy katalogów i plików, kolejność sklejania, komunikat przy uszkodzonym PDF; strażnik czasu Worda na atrapie COM (**zamyka tylko nasz proces**, nie zamyka niczego, gdy nowych jest kilka, po zawieszeniu nie startuje Worda drugi raz) i że **składanie nie zamraża reszty programu** |
...
| `test_uruchom.py` | że konsola chowa się **tylko po udanym starcie** i że obcy serwer na porcie nie uchodzi za nasz program; że **początek `start.bat` do wiersza aktualizacji jest zamrożony**, reszta to jeden blok, a nieudany `pip` nie kończy startu; że brak biblioteki to polski komunikat |

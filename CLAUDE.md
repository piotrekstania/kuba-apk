...
| `test_przegladarka.py` | układ mierzony w prawdziwej przeglądarce (Chrome/Chromium bez okna, `--dump-dom`): **kafelki przy składaniu PDF-a mają równą wysokość** niezależnie od długości nazwy pliku (w co najmniej dwóch rzędach — w jednym siatka wyrównuje wysokości sama i test niczego by nie odróżnił), **lista operatów nie wystaje poza stronę** przy 1280, 900 i 700 px, próbki koloru mieszczą się w kartach, a drugie „Zapisz” nie wysyła formularza drugi raz. Pomija się sam, gdy nie ma przeglądarki (i przy Chromium ze Snapa, który nie widzi `/tmp`); na runnerze CI Chrome jest |
| `test_zderzenia.py` | że to, co brat robi obok programu, nie psuje operatów: **numer wpisany z ręki nie wchodzi do cudzego katalogu**, „Powiel” go nie przenosi, dokument otwarty w Wordzie i PDF otwarty w czytniku dają komunikat, co zamknąć (a nie „literówkę w formatce” ani stronę błędu), **usuwanie przy otwartym pliku nie kasuje niczego**, zepsuta formatka dodatkowa zostawia stary plik i nie zjada numeru |

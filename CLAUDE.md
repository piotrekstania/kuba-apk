...
| **Poprawianie operatu sprząta dokumenty odznaczone w tej rundzie** (`operaty.usun_dokumenty_programu`) | poprawianie nadpisuje pliki nazwa po nazwie, więc dokument odznaczony w spisie treści (albo pominięty przez `wymaga`) zostawał w katalogu z danymi z poprzedniej rundy — szedł do scalonego PDF-a, choć spis o nim milczał, a przy pustym wykazie program ogłaszał „nie powstał”, mając jego starą wersję na dysku. Sprzątanie zna **wyłącznie nazwy nadawane przez program** (mapy i skany brata zostają), idzie **pod `_BLOKADA_PODGLADU`** — bo wątek podglądów listuje pliki przed blokadą i bez tego odtwarzałby PDF skasowanego dokumentu, a `unlink` pliku otwartego przez Worda dawałby `PermissionError` w środku trasy — i kasuje też podgląd. Dokument, którego wypełnienie **padło**, zostaje: stary plik jest lepszy niż dziura po awarii. Pliku nie do usunięcia (otwarty w Wordzie) nie przemilczamy — ostrzeżenie mówi, że wejdzie do PDF-a |
| **Ta sama sekcja w formularzu, o układzie w dokumencie decyduje znacznik w formatce** — `{%p for %}` daje kolejne strony, `{%tr for %}` kolejne wiersze jednej tabeli | o formie w przeglądarce decyduje **typ pola**, a o dokumencie **znacznik w formatce** — i to są dwie niezależne decyzje. Pole typu `sekcje` daje listę słowników, więc ta sama karta w formularzu może wyjść i jako strona, i jako wiersz — bez nowego typu pola i bez zmian w generatorze. Wykaz działki przeszedł tak z `{%tr for %}` (wiersze jednej tabeli) na `{%p for %}` (strona na działkę, 19.08.2026) **bez ruszania formularza** — wymieniła się sama formatka |
| **Wykaz działki wygląda dokładnie jak wykaz budynku: jedna działka = jedna strona** (decyzja z 19.08.2026, druga tego dnia) | poziomy układ z trzynastoma kolumnami zastąpiła najpierw pionowa tabela po dwa wiersze na działkę, ale brat obejrzał wydruk i wybrał to, co zna z wykazu budynku: atrybut w wierszu, dwie kolumny stanów, osobna strona na każdą działkę, numer działki w nagłówku **swojej** strony. Formatkę buduje z formatki budynku `narzedzia/utworz_wykaz_dzialki.py` — jeden wzorzec na oba dokumenty, więc nie rozjadą się przy następnej poprawce. Numer działki jest **podpolem każdego wykazu** (`dzialka`), a nie polem operatu: skoro każda działka ma własną stronę, to i własny numer. Nazywa się „Działka”, bo identyfikatorem jest dopiero całość — obręb, kropka i numer |
| **W wykazie działki OFU, OZU, OZK i PPU stoją obok siebie** (decyzja z 20.08.2026) | jeden użytek to komplet czterech wartości, a przy PPU jako osobnym, czwartym wierszu tabeli czytało się je w dwóch odległych miejscach — nie było widać, która powierzchnia należy do której klasy. Teraz każdy stan dzieli się na cztery podkolumny, a jeden użytek to jedna linijka we wszystkich naraz. Wiersze bez podziału (numer działki, pole powierzchni ewidencyjnej) mają wartość scaloną przez cztery podkolumny. Tabela ma **tę samą szerokość co tabela wykazu budynku** (8931 twipów) — oba dokumenty leżą w operacie obok siebie. Ten sam układ ma **formularz i strona operatu**: podpole dostaje `podkolumna` obok `wiersz` i `kolumna`, a `Pole.podkolumny` opisuje podział — dzięki temu wpisuje się tam, gdzie potem się czyta, i da się zestawić okiem formularz z gotowym dokumentem |
| **Stan nowy różny od dotychczasowego wchodzi do wykazu na czerwono i grubo** (`generator._zaznacz_zmiany`) | tak brat zaznaczał to dotąd ręcznie w Wordzie i po tym ośrodek czyta, co się zmieniło. Sygnałem jest `{{r … }}` w formatce — podmieniamy **tylko** te klucze, dla których dana formatka ma znacznik RichText, bo RichText w miejscu zwykłego `{{ }}` daje plik, którego Word nie otworzy (pułapka po `app/tekst.py`). Wpis tam, gdzie było pusto, to zmiana; puste pole w stanie nowym **nie** — brak wpisu nie znaczy „wykreślono”. Czerwień ta sama co numeru roboty (`FF0000`). **Jednostką zmiany jest wiersz tabeli, nie pojedyncza komórka** (decyzja z 24.08.2026): użytek to komplet OFU, OZU, OZK i PPU, więc gdy zmieni się choć jedna z tych wartości, cały stan nowy w tym wierszu idzie na czerwono. Przy znakowaniu komórka po komórce brat musiał dopisywać resztę ręcznie w Wordzie — wykaz z jedną czerwoną klasą wśród czarnych sąsiadów wygląda na pomyłkę. Grupę wyznacza **formatka**: klucze złapane w jednym `w:tr` to jedna rzecz, więc wiersze z jedną parą (wykaz budynku) zachowują się jak dotąd i nie trzeba niczego wpisywać do kodu |
| **OFU, OZU, OZK i pole powierzchni użytków są wielolinijkowe** (`typ: "textarea"` w podpolu), a **puste linijki na początku wartości zostają** (`_przytnij_wielolinijkowe` w `main.py`) | jedna działka bywa podzielona na kilka użytków i w wykazie stoją jeden pod drugim, po jednej wartości w linijce. Do dokumentu wchodzą tak samo: `\n` w `<w:t>` Word i LibreOffice łamią jak `<w:br/>` — na tym samym opiera się opis przebiegu prac. Puste linijki nie są śmieciem: brat zsuwa nimi wpis do właściwego wiersza sąsiedniej kolumny (zmiana dotycząca dopiero drugiego użytku) — `strip()` zjadał je po cichu. Ogonowe entery ucinamy nadal, a `\r\n` z przeglądarki schodzi do `\n`. **Komórki tych pól są w formatce wyrównane do góry** (prośba brata, 19.08.2026): środkowanie w pionie rozjeżdżało kolumny przy różnej liczbie linijek — krótsza pływała i linijki nie trzymały poziomu; ogonowych enterów nie trzeba dzięki temu w ogóle. **O wyrównaniu w pionie decyduje treść, nie formatka** (`generator.wyrownaj_komorki_stanow`, po wypełnieniu, przed zapisem): wiersz, w którym któraś komórka stanu ma kilka linijek, idzie do góry — inaczej krótsza kolumna pływa i puste linijki, którymi brat dosuwa wpis do właściwego użytku, przestają cokolwiek znaczyć. Wiersz z jedną wartością po każdej stronie wraca na środek, bo przy górnej krawędzi liczba nie trzymała poziomu z nazwą atrybutu obok (etykieta bywa dwuwierszowa). Formatka nie ma jak tego wiedzieć — liczba linijek jest w danych |
...
| W wykazie zmian działki numer za kropką wstawia **podpole `dzialka`** — `[{{ polozenie_obreb_teryt }}.{{ dzialka.dzialka }}]`, po jednym w każdej karcie | decyzja zmieniana dwa razy 19.08.2026: najpierw znacznik kończył się kropką i brat dopisywał numer ręcznie w Wordzie; potem numer dostał **jedno pole na cały wykaz** (`wykaz_identyfikator_dzialki`); odkąd każda działka ma własną stronę, tamto pole **zniknęło**, a numer jest podpolem karty — każda strona nosi swój. Nadal **nie jest** tym samym co `{{ nr_dzialki }}` z „Położenia”: tamto opisuje robotę, to konkretny wykaz. Puste podpole zostawia samą kropkę, numer dopisuje się wtedy w Wordzie. Stary klucz w danych operatu sprzed zmiany nigdy nie wraca do formularza — pokazuje się już tylko w „Pozostałych danych” |
...
| **Numer operatu wpisany z ręki, który ma już inny operat, nie przechodzi** (strażnik w `POST /generuj`) | nowy operat z cudzym numerem wchodził do katalogu tamtego: przepisywał mu `operat.json` i dokumenty, brał jego mapy i skany za swoje, a „Usuń” jednego kasowało potem katalog obu. Zajęty jest numer z katalogiem w `wyniki/` **albo z wpisem w historii** — operat przeniesiony do archiwum nie ma już katalogu, ale numer nadal jest jego. Numer z licznika jest zawsze świeży (wiersz niżej), więc pilnujemy tylko wpisanego; poprawka z numerem **własnego** operatu to nie zderzenie. Katalog bez `operat.json` (założony przez brata z góry, z mapami na nową robotę) nie jest cudzym operatem — numer z ręki trafia do niego jak dotąd. Z tego samego powodu „Powiel” nie przenosi już numeru wpisanego kiedyś z ręki: numer z licznika w danych i tak nie siedział, ale ręczny siedział i szedł do nowego operatu |
| **Plik otwarty w innym programie to komunikat, co zamknąć — nie awaria** | na Windowsie dokument otwarty w Wordzie i złożony PDF otwarty w czytniku blokują zapis (`PermissionError`). Dotąd „Popraw” szło przez komunikat o literówce w formatce (brat szukał błędu w szablonie, zamiast zamknąć Worda), a składanie kończyło się ogólną stroną błędu. **Usuwanie** zaczyna od zmiany nazwy całego katalogu: Windows odmawia jej, gdy w środku coś jest otwarte, a wtedy jeszcze nic nie zniknęło — `rmtree(ignore_errors=True)` zostawiał pół katalogu, często już bez `operat.json`, i kasował wpis z historii. Formatka dodatkowa, która się nie wczytuje, i plik, którego już nie ma przy składaniu, to ostrzeżenia, a nie wywrócona trasa. Na Linuksie blokadę udaje w testach atrybut „tylko do odczytu” (`tests/test_zderzenia.py`); prawdziwą trzeba raz zobaczyć na Windowsie (lista przed wydaniem) |
| **Licznik nigdy nie wydaje numeru nie większego niż najwyższy znany** (`generator.najwyzszy_znany_numer` → `db.nastepny_numer(co_najmniej=…)`) plus **ostatnia zapora** w `operaty.zaloz(nowy=True)` | licznik wiedział tylko o numerach, które sam wydał. **Nie trzeba było utraty bazy**: wystarczyło raz wpisać numer z ręki wyżej niż licznik, a licznik po dojściu do niego wchodził do cudzego katalogu, nadpisywał mu opis i spis treści, a w historii dwa wpisy z tym samym numerem wskazywały jeden katalog (odtworzone w teście, zanim powstała poprawka). Po utracie bazy to samo działo się już z 001. Teraz kolejny numer to jeden więcej niż najwyższy numer z bieżącego roku z licznika, z historii (także operaty w archiwum) i z katalogów w `wyniki/` (także skopiowanych z innego komputera); rozpoznaje je wzorzec pola (`{numer3}/{rok}`), więc „12/2026” bez zer też się liczy. **Świadomie „od najwyższego”, a nie „pierwszy wolny”** (decyzja z 23.09.2026): po ręcznym 050 licznik idzie od 051 i zostawia lukę, ale luka nic nie psuje, a „pierwszy wolny” po utracie bazy wydałby drugi raz numer operatu z archiwum. Szary numer w pustym polu liczy się tak samo. Katalog bez `operat.json` (założony z góry przez brata) się nie liczy — trafi do niego operat z tym numerem, jak dotąd. Strażnik numeru z ręki porównuje **po wartości** tym samym parserem (`generator.klucz_numeru`), bo „1/2026” obok „001/2026” to osobne katalogi, ale dla ośrodka jeden numer. Więcej niż sześć cyfr to literówka, nie numer — takich parser nie bierze (SQLite nie zmieściłby liczby i każdy następny operat padałby na przepełnieniu). Skoro przeskok jest nieodwracalny, formularz **pyta** (`confirm`), gdy numer z ręki jest o ponad 20 wyższy niż kolejny wolny — literówka „0122” zamiast „012” przesunęłaby numerację na resztę roku — ale tylko przy zmienionym polu, w tym samym roku i dla numeru do sześciu cyfr (inaczej licznika nie przesuwa i pytanie kłamałoby); odmowa zatrzymuje wysyłkę w całości (`stopImmediatePropagation`), inaczej blokada podwójnego „Zapisz” uznałaby formularz za wysłany. Katalogi licznik czyta po samych nazwach (katalog nazywa się numerem), bez otwierania każdego `operat.json` — to się dzieje przy każdym otwarciu formularza; opis czyta tylko dla katalogu, którego nazwa numerem nie jest („005.2026 Kowalski” po archiwizacji, kopia „- Kopia”). Zapora: nowy operat nigdy nie wchodzi do katalogu, w którym leży cudzy `operat.json` — rzuca `KatalogZajety`, niczego nie nadpisuje i oddaje numer. Przy okazji wyszło, że nieudany zapis nowego operatu zostawiał katalog z samym opisem (na liście jako „spoza historii”); `operaty.wycofaj_nowy` sprząta teraz to, co założył `zaloz` |
| **Zmiana numeru przy „Popraw” przenosi cały katalog** (`operaty.przenies`, `db.przenies_operat`) | dotąd poprawka z nowym numerem zakładała katalog obok: dokumenty szły do nowego, mapy brata zostawały w starym, historia mówiła stary numer i wskazywała nowy katalog, a na liście stały dwa wiersze ze starym numerem i żadnego z nowym. Teraz katalog zmienia nazwę w całości (z plikami brata, `operat.json`, ułożeniem kafelków i podglądami), **przed** wypełnianiem dokumentów — plik otwarty w Wordzie blokuje na Windowsie zmianę nazwy katalogu, a wtedy lepiej nie ruszyć niczego (formularz wraca z danymi i mówi, co zamknąć). Nowy numer idzie za katalogiem od razu — do historii, do `operat.json` (`operaty.ustaw_numer`) i do danych formularza, jeśli numer był wpisany z ręki — a formularz po błędzie pokazuje już nowy numer. Bez tego padnięte wypełnianie zostawiało `operat.json` ze starym numerem i następne zwykłe „Popraw” (puste pole) zakładało katalog obok, czyli rozbijało operat drugi raz; numer z ręki wracał do pola i przenosił katalog z powrotem. Katalog o nowej nazwie, który już istnieje, blokuje zmianę (dwóch katalogów nie łączymy) — a komunikat **nie radzi go usuwać**, bo to zwykle folder brata z mapami na inną robotę. Podpowiedź pod polem przy poprawianiu mówi teraz prawdę: puste pole zostawia numer. Złożony PDF jedzie z katalogiem pod tą samą nazwą, ale ma w środku dawny numer — strona operatu mówi, żeby złożyć go jeszcze raz |
| **Stary złożony PDF po zmianie numeru roboty jest usuwany, a nazwy złożonych PDF-ów program pamięta** (`operat.json` → `zlozone`) | kafelkiem nie był tylko PDF nazwany **obecnym** numerem roboty, więc po „Popraw” z nowym numerem stary złożony operat stawał się zwykłym kafelkiem, włączonym jak każdy — i cały wchodził do nowego PDF-a (na próbie 4 strony zamiast 2). Stary plik i tak jest nieaktualny (dawny numer i dawne dokumenty w środku), więc go usuwamy i mówimy o tym na stronie operatu — ale **dopiero po udanym zapisie dokumentu** (`operaty.usun_stary_wynik` w `main.generuj`): kasowany w `zaloz` znikał także wtedy, gdy poprawka padła (spis treści otwarty w Wordzie), a wiadomość przepadała razem z nią. Który plik jest „stary”, liczymy z numeru roboty, jaki **poprawiany wpis** miał w historii — nie z `operat.json`, bo ten bywa cudzy (wspólny katalog po dawnych błędach) i kasowalibyśmy PDF innego operatu — i usuwamy go **raz**, przy tej poprawce, która numer zmienia: wersja odłożona potem przez brata pod tą nazwą (np. wysłana do ośrodka) zostaje. `zlozone` **tylko ukrywa** kafelki, nic z niego nie jest kasowane — `zaloz` dopisuje tam stary wynik, więc nie jest kafelkiem także wtedy, gdy usunąć się go nie da (otwarty w czytniku), i także po złożeniu operatu pod nowym numerem. `zaloz` przepisuje `operat.json` od nowa i **musi przenosić `zlozone`**, jak `uklad` i `notatka`. Nie obejmuje starych PDF-ów, które już leżą w katalogach po takiej zmianie sprzed tej poprawki — tych program nie odróżni od plików brata |
| **Dwa wpisy w historii z jednym katalogiem: katalog należy do jednego z nich** (`main._wlasciciel_katalogu`) | tak zostawiały dane dawne błędy numeracji — licznik wchodził do cudzego katalogu, a numer wpisany z ręki mógł trafić w numer operatu z archiwum. Właścicielem jest wpis, którego numer roboty stoi w `operat.json` katalogu (przy remisie najnowszy — on zapisywał ostatni). Własność zapisuje się w `operat.json` jako **numer wpisu** (`wpis`: `zaloz` przy poprawianiu — przed dokumentem, bo własność sprawdza się wcześniej — i `operaty.oznacz_wpis` po zapisaniu nowego operatu) i to on rozstrzyga pierwszy: sam numer roboty zmienia się przy poprawianiu, więc nieudana poprawka przerzucała własność na drugi wpis. Na innym komputerze numer wpisu wskazuje obcą bazę: zwykle nie pasuje do żadnego z tutejszych wpisów i się nie liczy, a gdy wskazuje **inny, istniejący** wpis, katalog jest cudzy także wtedy, gdy w historii wskazuje go tylko jeden wpis — tak wygląda katalog skopiowany z laptopa na miejsce operatu, który tu poszedł do archiwum, i „Popraw” tutejszego nadpisałby tamten. „Popraw” **nie-właściciela** bez zmiany numeru dostaje odmowę z podpowiedzią kolejnego numeru, pod którym **nie ma jeszcze katalogu** (dotąd kasował tamtemu złożony PDF i nadpisywał opis i dokumenty), a z nowym numerem dostaje **własny** katalog — cudzego nie przenosi (dotąd przenosił mapy i skany tamtego pod swój numer) i nie wchodzi do katalogu, który już istnieje: literówka „001/2026.” to po zamianie na nazwę ten sam wspólny katalog, a folder założony przez brata z góry to mapy innej roboty. Własność ustala się **przed** strażnikiem numeru, który dla nie-właściciela nie uznaje wspólnego katalogu za „własny”. Właściciel zmieniający numer przenosi katalog w całości — także z plikami, które drugi wpis miał tam sprzed błędu — więc strona operatu mówi, który operat stoi teraz „w archiwum”. „Usuń” wpisu ze wspólnym katalogiem zdejmuje **sam wpis**: dwa wiersze z jednym numerem aż proszą, żeby jeden usunąć, a kasowanie katalogu zabrałoby i tamten operat; pytanie na liście mówi to wprost. Nowe dane takiego stanu już nie wytworzą (licznik i strażnik wyżej) |
| **Aktualizacja jest cała albo wcale** (`aktualizacja._zablokowane`, `_przywroc`, `WERSJA` kopiowana na końcu) | kopiowanie szło plik po pliku i wywracało się na pierwszym, którego nie dało się nadpisać — zwykle na formatce otwartej w Wordzie. Zostawało `app/` już nowe, a `WERSJA` i `requirements.txt` stare: program startował z nowym kodem bez nowych bibliotek, a komunikat mówił „nic nie zostało zepsute”. Teraz **przed** kopiowaniem sprawdzamy, czy każdy plik do nadpisania (i do skasowania w lustrzanym `szablony/`) da się otworzyć do zapisu — Windows odmawia tego dokładnie wtedy, kiedy odmówiłby nadpisania — i przy blokadzie nie ruszamy niczego, a komunikat mówi, który plik zamknąć. Kopia zapasowa powstaje dopiero wtedy, gdy naprawdę kopiujemy (brat przy otwartym Wordzie potrafi uruchomić program kilka razy pod rząd). Kopiowanie, które padnie w połowie z innego powodu (pełny dysk, antywirus), wraca do kopii zrobionej przed chwilą. `WERSJA` idzie na sam koniec: przerwanie, które nie zdąży posprzątać (zamknięte okno, zanik prądu), zostawia stary numer, więc następny start kopiuje wszystko jeszcze raz. Wyjątek **po** skopiowaniu całości (antywirus trzyma plik w katalogu tymczasowym i jego sprzątanie pada) niczego nie cofa — inaczej każdy start ściągałby, instalował i wycofywał to samo. **Pułapka 7b:** aktualizację wykonuje stary kod, więc to wszystko chroni dopiero aktualizacje **po** tej, która to przywiozła |
| **Nieudana instalacja bibliotek nie zatrzymuje startu** (`start.bat`, `uruchom._brakuje_bibliotek`) | `start.bat` kończył się po nieudanym `pip install` i program nie ruszał wcale — w terenie, bez internetu, wystarczyła do tego zmiana w `requirements.txt`, choć stare biblioteki zwykle wystarczają (najczęściej to podbita wersja, a nie nowa biblioteka). Teraz program startuje z tym, co jest, a bez znacznika `.venv\zainstalowane.txt` instalacja dokończy się przy następnym uruchomieniu z internetem. Gdy czegoś naprawdę brakuje, `uruchom.py` importuje `app.main` przed startem serwera i mówi po polsku, której biblioteki brak i że trzeba internetu — zamiast angielskiego śladu stosu. Łapie **każdy** `ImportError`, nie tylko `ModuleNotFoundError`: po nieudanym `pip` częściej zdarza się stara wersja biblioteki przy nowym kodzie („cannot import name …”); błąd importu modułu samego programu (`app.*`) przepuszcza, bo to usterka programu, a nie brak internetu. Kasuje przy tym znacznik `zainstalowane.txt`, żeby następny start z internetem na pewno instalował. Jak zmieniać sam `start.bat` — pułapka 37 |
| **Dane sekcji wyłączonej w formularzu zostają przy „Popraw”** (`main._dane_wylaczonych`, `main._pole_aktywne`) | wyłączonych pól przeglądarka nie wysyła, a zapis nadpisywał dane tym, co przyszło — odznaczenie w spisie treści np. wykazu budynku kasowało wszystkie jego wykazy, a przy ponownym zaznaczeniu karta była pusta. Teraz dane pól wyłączonych, z których przeglądarka **nie przysłała niczego**, przechodzą z poprzedniego wpisu do historii i `operat.json`, a formularz (także po błędzie walidacji) ma je na miejscu. **Do dokumentu nie wchodzą** — kontekst formatek dalej budujemy wyłącznie z tego, co przyszło — i strona operatu też ich nie pokazuje. O tym, co jest wyłączone, `_pole_aktywne` rozstrzyga **dokładnie tak jak skrypt formularza** (`odswiezZaleznosci` w `formularz.html`): łańcuch przełączników, pozycja „zawsze” liczona jak odznaczona (jest wyłączona), przełącznik, którego nie ma, nie wyłącza niczego — zmieniając jedno, zmień drugie. Pole, które przyszło (choćby puste: przeglądarka bez skryptów niczego nie wyszarza), wygrywa, bo brat mógł je świadomie wyczyścić. Gdzie serwer nie wie na pewno, co przeglądarka wyłączyła, uznaje pole za włączone: tabeli i pola TERYT formularz w ogóle nie wyszarza, `klucz:on` nie pasuje do pola wyboru bez `value`, a to, które pozycje listy „dokumenty” formularz pokazuje, zależy od szablonów — takie pola zachowują się jak dawniej. Nowy operat i „Powiel” niczego nie zachowują — mają tylko to, co przyszło |
| **Lista wyboru pokazuje zapisaną wartość spoza dzisiejszych opcji** (`formularz.html`, pole `select`) | opcje siedzą w `.json` formatki i zmieniają się z wersjami. Przeglądarka zaznaczała wtedy pierwszą opcję, więc samo „Popraw” + „Zapisz” po cichu zmieniało np. rodzaj pracy. Karta sekcji (`pole_sekcji`) robiła to od dawna, zwykła lista nie |
| **Word ma limit czasu: po 3 minutach jednego kroku strażnik zamyka naszą instancję** (`pdf._Straznik`, `LIMIT_WORDA`) | Word bez okna potrafi stanąć na pytaniu, którego nikt nie widzi (aktywacja Office, naprawa pliku). Konwersja czekała wtedy w nieskończoność pod `_BLOKADA_KONWERSJI` — stawały podglądy i każde następne składanie, aż do zamknięcia programu, a w tle zostawał `WINWORD.EXE`. Zamykamy wyłącznie proces, który po naszym `DispatchEx` pojawił się jako **jedyny nowy** (numery z `tasklist`, zdjęcie przed startem i po nim): dwa nowe naraz to znak, że brat właśnie otworzył swojego Worda, i wtedy nie zamykamy niczego — lepiej czekać niż zamknąć mu niezapisany dokument. Limit liczy się na krok (start z pierwszym dokumentem, potem każdy dokument) z ogromnym zapasem wobec zwykłych 1,5 s. Po zawieszeniu wsad nie próbuje dokumentów pojedynczo Wordem — każdy start stanąłby na tym samym oknie — tylko LibreOffice'em, jeśli jest. **Zawieszenie jest pamiętane** (`PRZERWA_PO_ZAWIESZENIU`, 10 minut): robota w tle — podglądy po „Zapisz” i miniatury na stronie składania (`w_tle=True`) — Worda wtedy nie uruchamia (LibreOffice, jeśli jest, albo od razu zastępnik), bo każda miniatura czekałaby pełny limit, a za nimi „Złóż PDF” — przy czterech dokumentach kwadrans. Składanie, o które brat prosi wprost, próbuje zawsze, a udana konwersja pamięć kasuje. Pod limitem jest też zamykanie Worda, a strażnik, który przy starcie nie znał numeru procesu, szuka go przy następnym odliczeniu, zamiast odpuścić. Kosztuje to dwa wywołania `tasklist` na jedno uruchomienie Worda. Na prawdziwym Wordzie sprawdzają to `test_limit_czasu_*` w `test_word.py` |
| **Składanie PDF-a i zapis operatu chodzą w puli wątków** (`run_in_threadpool` w `POST /scal/{nazwa}` i `POST /generuj/...`) | trasy `async` wołały konwersję, sklejanie i czekanie na blokadę podglądów wprost w pętli zdarzeń, więc dopóki Word pracował — a przy zawieszonym aż do limitu — serwer nie odpowiadał na nic, nawet na otwarcie Pomocy w drugiej karcie, i program wyglądał na zawieszony. Zapisy operatów idą przy tym **po kolei** (`_BLOKADA_ZAPISU`), tak jak wtedy, gdy pętla zdarzeń nie pozwalała im się przepleść — numeracja i zakładanie katalogów tego wymagają. Pozostałe trasy z konwersją są zwykłymi `def`, które FastAPI i tak puszcza w puli. **Trasa `async` nie woła niczego, co może trwać** |
...
| `narzedzia/instalacja_testowa.py` | odtwarza instalację brata (zip z GitHuba, bez `.git`), opcja `--stara-wersja` wymusza aktualizację przy starcie, a `--z-commita <sha>` buduje ją ze **starego** kodu — tylko tak sprawdzisz przejście ze starego `start.bat` i aktualizatora na nowe, gdy nowy kod jest już na `main` |
...
27. **Parser HTML ignoruje pierwszy znak nowej linii po `<textarea>`.** Wartość
   ...
   siedział po stronie serwera: `strip()` w `odczytaj_dane` — stąd
...
35. **Numer z licznika nie może być w teście wpisany razem z rokiem.** Licznik liczy
   w bieżącym roku, więc asercja `== "001/2026"` przechodzi do 31 grudnia, a 1 stycznia
   czerwienieje bez żadnej zmiany w kodzie — i to akurat wtedy, gdy nikt nie pamięta,
   skąd się wzięła. Numery z licznika buduj z `date.today().year` (`ROK` w
   `test_numeracja.py` i `test_zderzenia.py`); numer **wpisany z ręki** może mieć rok
   na sztywno, byle test nie zakładał, że licznik go zna albo nie zna.

36. **Poprawny JSON to nie zawsze słownik.** `json.loads` na `operat.json` z `[]` albo
   `null` (ręczna edycja, przerwany zapis) nie rzuca wyjątku, tylko oddaje listę albo
   `None` — a każde wywołanie woła potem `.get`. Dopóki opisy katalogów czytała sama
   lista operatów, jeden taki plik wywracał stronę główną; odkąd czyta je też licznik
   numerów, wywracał także formularz i zapis nowego operatu. `operaty.opis` oddaje
   więc pusty słownik dla wszystkiego, co słownikiem nie jest.

37. **cmd czyta `start.bat` po kawałku, a aktualizator podmienia go w trakcie.** cmd
   wykonuje plik wsadowy linijka po linijce, za każdym razem sięgając do pliku na dysku,
   i pamięta tylko, na którym **bajcie** skończył. Aktualizator jest wołany **z**
   `start.bat` i ten plik nadpisuje — po jego wyjściu cmd czyta dalej od tego samego
   bajtu, ale już w nowym pliku. Dlatego wszystko do wiersza
   `.venv\Scripts\python -m app.aktualizacja` włącznie jest **zamrożone bajt w bajt**
   (skrót sprawdza `test_poczatek_start_bat_do_aktualizacji_jest_zamrozony`): zmiana
   choćby komentarza przed tym wierszem przesunęłaby bajty i cmd trafiłby w środek
   jakiejś linijki nowego pliku. Gdyby początek naprawdę trzeba było zmienić — w dwóch
   wydaniach: najpierw nowy kod za tym wierszem, dopiero potem porządki przed nim.
   Reszta to **jeden blok w nawiasach**, który cmd wczytuje w całości przed wykonaniem,
   zakończony `exit /b` — potem cmd do pliku już nie sięga, więc nie przeszkadza mu
...
38. **Sprawdzając, czy test łapie błąd, pilnuj plików `.pyc`.** Python uznaje
   ...
   sekundy i jego rozmiar**. Dwie kolejne „mutacje” tej samej długości zapisane w tej
   samej sekundzie (np. `if isinstance(blad, X):` → `if False:` w dwóch miejscach tego
   samego pliku) dają więc test uruchomiony na kodzie **poprzedniej** mutacji — zielony
   wynik, choć test działa. Wyszło przy sprawdzaniu strażnika Worda. Puszczając testy na
   zepsutym kodzie, ustaw `PYTHONPYCACHEPREFIX` na świeży katalog przy każdym
   uruchomieniu, a po wszystkim skasuj `__pycache__`.

39. **Promień 999 px i sprężyna nie idą w parze.** Pastylka zmieniająca kształt pod
   kursorem albo przy wciśnięciu (`999px` → `12px`) animuje się sprężyną
   (`--sprezyna`, przestrzelenie ok. 10% drogi). Przy drodze 987 px już 1% przestrzelenia
   to wartość ujemna, którą przeglądarka przycina do zera — rogi na chwilę robiły się
   **ostre**, zanim się zaokrągliły (brat zauważył to na zielonej plakietce „PDF: …”).
   Drugi skutek tego samego: gdy suma promieni na boku przekracza jego długość,
   przeglądarka skaluje w dół **wszystkie cztery** rogi tym samym współczynnikiem — więc
   przy `999px 6px 6px 999px` wewnętrzne rogi przycisków w grupie wychodziły ostre zamiast
   lekko zaokrąglonych. Dlatego pastylka, która zmienia kształt, ma promień z własnej
   wysokości (pół wysokości albo trochę więcej); wyższe przyciski podają go przez
   `--pastylka` — ustawiony wprost `border-radius` z mocniejszym selektorem przebiłby
   `:active` i przycisk przestałby reagować kształtem. `999px` zostaje tylko tam, gdzie
   nic się nie animuje. Pilnuje tego `test_rogi_nie_ostrzeja_w_trakcie_zmiany_ksztaltu`:
...
oraz wykazy zmian danych ewidencyjnych dotyczące budynku i działki — oba pionowe
i zbudowane tak samo (od 19.08.2026 wykaz działki powstaje z formatki budynku,
jedna działka na stronę). Wygląd sprawdzony na Wordzie
u brata — po serii poprawek,
...
| `test_aktualizacja.py` | że `dane/` i `wyniki/` przeżywają, szablony są lustrzane, a nieudana aktualizacja zostawia działający program (GitHub podstawiony przez `file://`); że **plik otwarty w innym programie nie zostawia połowicznej aktualizacji**, awaria w połowie wraca do poprzedniej wersji, a przerwana nie zmienia numeru w `WERSJA`; że nieudane sprzątanie po udanym kopiowaniu nie cofa aktualizacji |
| `test_db.py` | migracje starej bazy bez utraty historii i licznika; że kopia powstaje **przed** migracją i tylko wtedy, a nie przy zakładaniu bazy od zera; że połączenie **zamyka plik** (inaczej Windows blokuje bazę); że sprzątanie kopii bazy liczy czas, a nie kolejność liter („schemat10” < „schemat2”) |
...
| `test_operaty.py`, `test_pdf.py` | nazwy katalogów i plików, kolejność sklejania, komunikat przy uszkodzonym PDF; strażnik czasu Worda na atrapie COM (**zamyka tylko nasz proces**, nie zamyka niczego, gdy nowych jest kilka, pilnuje też zamykania Worda, po zawieszeniu nie startuje go drugi raz, a **przez 10 minut podglądy i miniatury go nie ruszają**, choć składanie próbuje) i że **składanie ani zapis operatu nie zamrażają reszty programu**, a zapisy dalej idą po kolei |
...
| `test_uruchom.py` | że konsola chowa się **tylko po udanym starcie** i że obcy serwer na porcie nie uchodzi za nasz program; że **początek `start.bat` do wiersza aktualizacji jest zamrożony**, reszta to jeden blok, a nieudany `pip` nie kończy startu; że brak biblioteki — także za stara wersja — to polski komunikat, a błąd importu w samym programie nie udaje braku biblioteki |
...
| `test_przegladarka.py` | układ mierzony w prawdziwej przeglądarce (Chrome/Chromium bez okna, `--dump-dom`): **kafelki przy składaniu PDF-a mają równą wysokość** niezależnie od długości nazwy pliku (w co najmniej dwóch rzędach — w jednym siatka wyrównuje wysokości sama i test niczego by nie odróżnił), **lista operatów nie wystaje poza stronę** przy 1280, 900 i 700 px, próbki koloru mieszczą się w kartach, drugie „Zapisz” nie wysyła formularza drugi raz, a numer z ręki dużo wyższy niż kolejny wymaga potwierdzenia (odmowa nie blokuje formularza); że **rogi nie ostrzeją w trakcie zmiany kształtu**, a pastylki zostają pastylkami (pułapka 39). Pomija się sam, gdy nie ma przeglądarki (i przy Chromium ze Snapa, który nie widzi `/tmp`); na runnerze CI Chrome jest |
| `test_instalacja_testowa.py` | że instalacja testowa z `--z-commita` powstaje z paczki starego commita i ma cofnięty numer — inaczej `start.bat` uznałby ją za aktualną i przejścia nie dałoby się sprawdzić |
| `test_poprawianie.py` | że „Popraw” nie gubi danych: **wartość listy spoza dzisiejszych opcji** zostaje, a **dane sekcji wyłączonej w spisie treści** przeżywają zapis (w historii, w `operat.json` i w formularzu po błędzie), choć do dokumentu i na stronę operatu nie wchodzą; `_pole_aktywne` czyta przełączniki tak jak skrypt formularza, a tam, gdzie przeglądarka pola nie wyłącza (tabela, TERYT, `klucz:on`, lista „dokumenty”), nie zgaduje |
| `test_zderzenia.py` | że to, co brat robi obok programu, nie psuje operatów: **numer wpisany z ręki nie wchodzi do cudzego katalogu**, „Powiel” go nie przenosi, dokument otwarty w Wordzie i PDF otwarty w czytniku dają komunikat, co zamknąć (a nie „literówkę w formatce” ani stronę błędu), **usuwanie przy otwartym pliku nie kasuje niczego**, zepsuta formatka dodatkowa zostawia stary plik i nie zjada numeru, nieudany nowy operat nie zostawia katalogu z samym opisem, a **stary złożony PDF po zmianie numeru roboty nie wchodzi do nowego** (także gdy jest otwarty w czytniku i usunąć się go nie da) |
| `test_numeracja.py` | że **licznik nie wydaje numeru, który ktoś już ma** — wpisanego kiedyś z ręki (także bez zer: „1/2026” to „001/2026”), z historii (także operatu w archiwum), z katalogu po utracie bazy — i że szary numer w formularzu to ten, który operat naprawdę dostanie; że zapora nie wpuszcza nowego operatu do cudzego katalogu; że **zmiana numeru przy „Popraw” przenosi cały katalog** (z mapami i ułożeniem kafelków), przy otwartym pliku nie rusza niczego, a po padniętym wypełnianiu następna poprawka nie rozbija operatu; że na danych po dawnych błędach (**dwa wpisy, jeden katalog**) „Popraw” i „Usuń” nie ruszają operatu, do którego katalog należy — także przy literówce w numerze, przy folderze założonym przez brata i po nieudanym zapisie; że stary złożony PDF znika raz, przy zmianie numeru roboty **tego** wpisu |
...
1. **Dokładanie kolejnej formatki** (przepis, bo to się będzie powtarzać): wrzuć `.docx`
   ...
   Orientacja nie wymaga niczego dodatkowego: szerokości liczą się z rozmiaru

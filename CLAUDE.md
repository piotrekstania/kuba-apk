...
| **OFU, OZU, OZK i pole powierzchni użytków są wielolinijkowe** (`typ: "textarea"` w podpolu) | jedna działka bywa podzielona na kilka użytków i w wykazie stoją jeden pod drugim, po jednej wartości w linijce. Do dokumentu wchodzą tak samo: `\n` w `<w:t>` Word i LibreOffice łamią jak `<w:br/>` — na tym samym opiera się opis przebiegu prac |

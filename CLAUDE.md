...
| **W wykazie działki OFU, OZU, OZK i PPU stoją obok siebie** (decyzja z 20.08.2026) | jeden użytek to komplet czterech wartości, a przy PPU jako osobnym, czwartym wierszu tabeli czytało się je w dwóch odległych miejscach — nie było widać, która powierzchnia należy do której klasy. Teraz każdy stan dzieli się na cztery podkolumny, a jeden użytek to jedna linijka we wszystkich naraz. Wiersze bez podziału (numer działki, pole powierzchni ewidencyjnej) mają wartość scaloną przez cztery podkolumny. Tabela ma **tę samą szerokość co tabela wykazu budynku** (8931 twipów) — oba dokumenty leżą w operacie obok siebie |

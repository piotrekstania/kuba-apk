...
| **Stan nowy różny od dotychczasowego wchodzi do wykazu na czerwono i grubo** (`generator._zaznacz_zmiany`) | tak brat zaznaczał to dotąd ręcznie w Wordzie i po tym ośrodek czyta, co się zmieniło. Sygnałem jest `{{r … }}` w formatce — podmieniamy **tylko** te klucze, dla których dana formatka ma znacznik RichText, bo RichText w miejscu zwykłego `{{ }}` daje plik, którego Word nie otworzy (pułapka po `app/tekst.py`). Wpis tam, gdzie było pusto, to zmiana; puste pole w stanie nowym **nie** — brak wpisu nie znaczy „wykreślono”. Czerwień ta sama co numeru roboty (`FF0000`) |

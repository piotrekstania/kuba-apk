...
| W wykazie zmian działki numer za kropką wstawia **pole `wykaz_identyfikator_dzialki`** — `[{{ polozenie_obreb_teryt }}.{{ wykaz_identyfikator_dzialki }}]` | **decyzja zmieniona 19.08.2026**: wcześniej znacznik kończył się kropką i brat dopisywał numer ręcznie w Wordzie, bo w jednym wykazie bywa ich kilka. Odkąd wykaz wypełnia się w programie, ręczna edycja przestała mieć sens i numer dostał własne pole. Jest **jedno na cały wykaz**, niezależnie od liczby wierszy — stoi w nagłówku, czyli poza pętlą `{%tr for %}`. Nadal **nie jest** tym samym co `{{ nr_dzialki }}` z „Położenia”: tamto opisuje robotę, to konkretny wykaz. Puste pole zostawia samą kropkę, czyli stan sprzed zmiany |
